...
type BlockList struct {
	...
	// version is a monotonic counter bumped under mu every time a
	// snapshot is taken. It lets persist() drop a stale snapshot
	// whose newer sibling has already reached disk (see persist).
	version uint64
	...
	// lastPersisted is the version of the most recent snapshot that
	// successfully reached disk. Guarded by saveMu.
...
// snapshot so the disk write can run without holding b.mu. It also
// bumps and stamps b.version so persist() can tell newer snapshots
// from older ones. Caller must hold b.mu for writing (the version
// bump mutates shared state); every mutation path already does.
...
// intermediate.
//
// Snapshots are taken in a total order under mu, but persist() is
// called after mu is released and only serialized by saveMu, so two
// concurrent mutations can reach the rename in either order. Without
// a guard, an older snapshot could land *after* a newer one and roll
// the on-disk file backwards (memory stays correct; the file goes
// stale until the next write). The version check drops any snapshot
// that is not newer than the last one persisted: the highest-version
// snapshot always wins, the file converges to the latest state, and
// superseded writes are coalesced away.
//
// I/O errors are logged but otherwise ignored to preserve best-effort
// save behaviour: a transient disk failure must not knock a working
// blocklist out of memory.
func (b *BlockList) persist(s blockSnapshot) {
	b.saveMu.Lock()
	defer b.saveMu.Unlock()

	// A newer snapshot has already reached disk (or this exact one
	// has); nothing to do.
	if s.version != 0 && s.version <= b.lastPersisted {
		return
	}
	...
	if err != nil {
		zlog.Warn("Blocklist persist failed: create temp", "dir", b.cfg.BlockListDir, "error", err.Error())
		return
	}
	tmpName := tmp.Name()
	...
	if err := tmp.Close(); err != nil {
		zlog.Warn("Blocklist persist failed", "stage", "close", "error", err.Error())
		cleanup()
		return
	}
	if err := os.Rename(tmpName, path); err != nil {
		zlog.Warn("Blocklist persist failed", "stage", "rename", "error", err.Error())
		cleanup()
		return
	}

	b.lastPersisted = s.version
}
