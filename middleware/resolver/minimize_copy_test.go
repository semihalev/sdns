...
func TestMinimizeCopiesOnlyWhenMinimizing(t *testing.T) {
	...
	if got, min := (&Resolver{}).minimize(req, 0, 0, false); got != req || min {
		t.Fatalf("disabled: got %p minimized=%v, want the request itself", got, min)
	}
	if got, min := r.minimize(req, 0, 0, true); got != req || min {
	...
	if got, min := r.minimize(req, 0, 5, false); got != req || min {
	...
	if got, min := r.minimize(req, 4, 0, false); got != req || min {
	...
	got, min := r.minimize(req, 1, 0, false)
	if !min || got == req {
